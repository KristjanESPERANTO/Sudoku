...
public class ToplevelActivity extends Activity {
	...
	private static final String TAG = "gnome-android";

	private static void dispatchGtkLayout(Runnable runnable) {
		/* Post to the GLib main context instead of running directly on a
		 * background thread.  The GDK Android visibility idle is scheduled at
		 * G_PRIORITY_DEFAULT-10 (-10); posting layout here at G_PRIORITY_DEFAULT
		 * (0) guarantees the visibility idle always runs first, so that
		 * delayed_map is set to TRUE before notifyLayoutSurface checks it.
		 * Without this ordering gdk_android_surface_handle_map is never called
		 * for fullscreen (x=0, y=0) toplevels and the window stays unmapped. */
		runOnGtkMainSafely(runnable);
	}

	/** Guards GlibContext.runOnMain so callers survive when the native bridge
	 *  is not yet loaded (e.g. fallback-mode / GTK4 not linked). */
	private static void runOnGtkMainSafely(Runnable runnable) {
		try {
			GlibContext.runOnMain(runnable);
		} catch (UnsatisfiedLinkError err) {
			Log.w(TAG, "toplevel:skip GTK main dispatch (native bridge unavailable)");
		}
	}
	...
	public class ToplevelView extends ViewGroup {
		public class Surface extends SurfaceView implements SurfaceHolder.Callback {
			...
			public Surface(long identifier) {
				...
				setVisibility(GONE);
				Log.i(TAG, "toplevel:surface ctor id=" + identifier);

				try {
					bindNative(identifier);
					this.surfaceIdentifier = identifier;
					Log.i(TAG, "toplevel:surface bindNative ok id=" + identifier);
				} catch (UnregisteredSurfaceException err) {
					logger.log(Level.WARNING, err.getMessage());
					Log.e(TAG, "toplevel:surface bindNative failed id=" + identifier + " err=" + err.getMessage());
					drop();
				}
			...
			public void setVisibility(boolean visible) {
				Log.i(TAG, "toplevel:setVisibility request visible=" + visible + " id=" + surfaceIdentifier);
				runOnUiThread(() -> {
					super.setVisibility(visible ? VISIBLE : GONE);
					Log.i(TAG, "toplevel:setVisibility applied visible=" + (getVisibility() == VISIBLE) + " id=" + surfaceIdentifier);
				});
			}
			...
			private boolean motionEventProxy(MotionEvent event) {
				...
				runOnGtkMainSafely(() -> notifyMotionEvent(identitiy, copy));
				return true;
			}
			...
			private boolean keyEventProxy(KeyEvent event) {
				...
				if (event.getKeyCode() == KeyEvent.KEYCODE_BACK)
					return false;
				runOnGtkMainSafely(() -> notifyKeyEvent(event));
				return true;
			}
			...
			protected void onAttachedToWindow() {
				...
				runOnGtkMainSafely(this::notifyAttached);
			}

			@Override
			protected void onDetachedFromWindow() {
				super.onDetachedFromWindow();
				runOnGtkMainSafely(this::notifyDetached);
			}

			@Override
			protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
				if (changed)
					runOnGtkMainSafely(() -> notifyLayoutPosition(
			...
			public void surfaceCreated(@NonNull SurfaceHolder holder) {
				Log.i(TAG, "toplevel:surfaceCreated id=" + surfaceIdentifier);
				/* Call notifyVisibility directly — this blocks on the GLib
				 * visibility idle barrier, which sets delayed_map=TRUE inside
				 * gdk_android_surface_eventloop_idle.  surfaceChanged fires
				 * only after this returns, so notifyLayoutSurface will always
				 * see delayed_map=TRUE and call gdk_android_surface_handle_map. */
				Log.i(TAG, "toplevel:notifyVisibility(true) dispatch");
				notifyVisibility(true);
				Log.i(TAG, "toplevel:notifyVisibility(true) done");
			}
			...
			public void surfaceChanged(@NonNull SurfaceHolder holder, int format, int width, int height) {
				...
				Log.i(TAG, "toplevel:surfaceChanged id=" + surfaceIdentifier + " size=" + width + "x" + height + " scale=" + scale);
				dispatchGtkLayout(() -> {
					Log.i(TAG, "toplevel:notifyLayoutSurface dispatch size=" + width + "x" + height + " scale=" + scale);
					notifyLayoutSurface(width, height, scale);
					Log.i(TAG, "toplevel:notifyLayoutSurface done size=" + width + "x" + height);
				});
			}

			@Override
			public void surfaceDestroyed(@NonNull SurfaceHolder holder) {
				Log.i(TAG, "toplevel:surfaceDestroyed id=" + surfaceIdentifier);
				Log.i(TAG, "toplevel:notifyVisibility(false) dispatch");
				notifyVisibility(false);
				Log.i(TAG, "toplevel:notifyVisibility(false) done");
			}
		}
		...
		protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
			...
			int childWidth = Math.max(0, width - (insets.left + insets.right));
			int childHeight = Math.max(0, height - (insets.top + insets.bottom));
	...
	protected final ToplevelView getToplevelViewContainer() {
		if (this.view == null)
	...
	private void attachToplevelSurface() {
		Log.i(TAG, "toplevel:attachToplevelSurface nativeId=" + this.nativeIdentifier);
		...
		runOnUiThread(() -> {
			...
			Log.i(TAG, "toplevel:surface added to container childCount=" + this.view.getChildCount());
	...
	private void updateToplevelState() {
		...
		runOnGtkMainSafely(() -> notifyStateChange(has_focus, is_fullscreen));
	}

	@Override
	public void onBackPressed() {
		runOnGtkMainSafely(this::notifyOnBackPress);
	}

	@Override
	public void onConfigurationChanged(@NonNull Configuration newConfig) {
		int oldNightMode = getResources().getConfiguration().uiMode & Configuration.UI_MODE_NIGHT_MASK;
		super.onConfigurationChanged(newConfig);
		int newNightMode = newConfig.uiMode & Configuration.UI_MODE_NIGHT_MASK;
		if (oldNightMode != newNightMode) {
			runOnGtkMainSafely(this::notifyConfigurationChange);
		}
	}
	...
	protected void onDestroy() {
		if (isFinishing())
			runOnGtkMainSafely(this::notifyDestroy);
		super.onDestroy();
	}

	@Override
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		runOnGtkMainSafely(() -> notifyActivityResult(requestCode, resultCode, data));
	}
